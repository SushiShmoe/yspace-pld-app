...
/**
   Request process list, print result to a FILE (e.g. stdout).
   @note This is currently only supported on FreeRTOS systems.
   @param[in] node address of subsystem.
   @param[in] timeout timeout in mS to wait for replies. The function will not return until the timeout occurrs.
*/
void csp_ps_to_file(uint8_t node, uint32_t timeout, FILE *out);


typedef void (*csp_ps_callback_t)(const char *);

/**
   Request process list with a callback function to use instead of writing to a file.

   @note This is currently only supported on FreeRTOS systems.
   @param[in] node address of subsystem.
   @param[in] timeout timeout in mS to wait for replies. The function will not return until the timeout occurrs.
*/
void csp_ps_with_callback(uint8_t node, uint32_t timeout, csp_ps_callback_t callback);
