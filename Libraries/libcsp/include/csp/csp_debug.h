...
#include <csp/csp_types.h>
...
#if (CSP_USE_CUSTOM_PRINTF)
    extern size_t csp_vsnprintf(char *str, size_t count, const char *fmt, va_list args);
    extern size_t csp_snprintf(char *str, size_t count,const char *fmt,...) __attribute__ ((format (__printf__, 3, 4)));
    extern size_t csp_vasprintf(char **ptr, const char *format, va_list ap);
    extern size_t csp_asprintf(char **ptr, const char *format, ...) __attribute__ ((format (__printf__, 2, 3)));
    extern size_t csp_sprintf(char *ptr, const char *format, ...) __attribute__ ((format (__printf__, 2, 3)));
    extern size_t csp_printf(const char *format, ...) __attribute__ ((format (__printf__, 1, 2)));
    extern size_t csp_vprintf(const char *format, va_list ap);
    ...
    #undef sprintf
    #undef printf
    #define vprintf csp_vprintf
    #define vsnprintf csp_vsnprintf
    #define snprintf csp_snprintf
    #define vasprintf csp_vasprintf
    #define asprintf csp_asprintf
...
typedef enum {
   ...
   CSP_HEXDUMP	= 7, //!< Dump Packet contents
   CSP_DEBUG_LEVEL_LAST = 8, //!< Not actual debug level, just a terminator
} csp_debug_level_t;
 ...
 * @param format log message, printf syntax.
 */
#define csp_log_hexdump(...)     { if (CSP_DEBUG && csp_debug_level_enabled[CSP_HEXDUMP]) csp_hex_dump(__VA_ARGS__); }
