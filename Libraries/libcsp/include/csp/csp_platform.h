...
/* Set OS */
	...
	#if defined(ESP_PLATFORM)
		#define CSP_DEFINE_CRITICAL(lock) static portMUX_TYPE lock = portMUX_INITIALIZER_UNLOCKED
		#define CSP_INIT_CRITICAL(lock) ({CSP_ERR_NONE;})
		#define CSP_ENTER_CRITICAL(lock) do { portENTER_CRITICAL(&lock); } while (0)
		#define CSP_EXIT_CRITICAL(lock) do { portEXIT_CRITICAL(&lock); } while (0)	
	#else
