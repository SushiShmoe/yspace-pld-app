...
/* srand() and rand() link to malloc. This is a workaround to prevent that. */
#if !CSP_HAVE_RAND
static bool s_random_port_seeded = false;
static uint32_t s_random_port_state = 0;
/* https://en.wikipedia.org/wiki/Lehmer_random_number_generator */
static int random_sport() {
    if (!s_random_port_seeded) {
        // Seed the first time it's used
        s_random_port_seeded = true;
        s_random_port_state = csp_get_ms();
    }
    uint64_t product = (uint64_t) s_random_port_state * 48271;
    uint32_t x = (product & 0x7fffffff) + (product >> 31);
    x = (x & 0x7fffffff) + (x >> 31);
    s_random_port_state = x;
    return (int)x & 0x7FFFFFFF; // This is not strictly needed, just making sure the number is positive
}
#endif
...
#else
    sport = (random_sport() % (CSP_ID_PORT_MAX - csp_conf.port_max_bind)) + (csp_conf.port_max_bind + 1);
#endif
