...
void csp_ps_with_callback(uint8_t node, uint32_t timeout, csp_ps_callback_t callback) {
    ...
    while(1) {
        ...
        callback((const char*)packet->data);
...
static void callback(const char *str) {
		...
		fputs(str, callback_file);
}

void csp_ps(uint8_t node, uint32_t timeout) {
    callback_file = stdout;
    csp_ps_with_callback(node, timeout, callback);
}

void csp_ps_to_file(uint8_t node, uint32_t timeout, FILE *file) {
    callback_file = file;
    csp_ps_with_callback(node, timeout, callback);
}
